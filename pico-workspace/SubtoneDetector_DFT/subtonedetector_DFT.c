/* Raspberry Pico Pi subtone DFT detector demo */
...
const double subtone_frequency = 123.0;                     // subtone-frequency to be detected

const int number_of_samples = 2000;                         // number of samples to be processed

volatile float sample_frequency = 13333.3333;               // ADC sample rate in Hz

volatile double adcval;

/*----- DFT ------*/
volatile double adcval_signed;                              // signed and normalized ADC-Value
volatile double amplitude;                                  // processed amplitude of subtone
...
void timer_irq_handler() 
{
    ...
    adcval = (double) adc_fifo_get();                       // fetch ADC-value from fifo

    adcval_signed = (adcval - 2047.0) / 2048.0;             // shift to +/- and normalize 12 bit ADC - Signal

    process_dft_sample(adcval_signed);                      // process subtone DFT detection
...
int main ()
{
    ...
    initialize_dft(subtone_frequency,sample_frequency,number_of_samples);     // init 123 Hz subtone detector
    ...
    while(1)                                                        // main loop 
    {
        amplitude = get_dft_amplitude();                            // get amplitude of subtone signal

        if(amplitude > 55.0)                                        // threshold
