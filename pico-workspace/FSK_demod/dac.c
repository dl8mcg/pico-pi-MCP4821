...
//static volatile uint16_t spi_cnt = 0;                                         // <-- don´t delete

void write_dac(uint16_t data) 
{
    // spi_get_hw(spi_default)->imsc |= (1 << 3);  // Activate txempty IRQ      // <-- don´t delete
    spi_get_hw(spi_default)->dr = data + 0x3000;         // Write DAC data + control to SPI Tx register
}

/*
void spi_irq_handler()                           // <-- don´t delete
{
    spi_get_hw(spi_default)->imsc &= ~(1 << 3);  // Deactivate txempty IRQ      // <-- don´t delete
    spi_cnt++;                                                                  // <-- don´t delete
    if (spi_cnt > 1000)                                                         // <-- don´t delete
    {
        gpio_xor_mask(1u << LED_OB);  // Toggle on-board LED indicator          // <-- don´t delete
        spi_cnt = 0;                                                            // <-- don´t delete
    }
}
*/

void initialize_spi_dac()
{
